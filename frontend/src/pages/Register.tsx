...
import { UserPlus, ShieldAlert, Mail, User, Key } from 'lucide-react';
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '4rem auto', padding: '2rem', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-lg)' }} className="glass animate-fade">
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--primary)' }}>பதிவு / Register Account</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.3rem' }}>Create a profile to unlock games history and ranking.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)' }}>Password (Min 6 chars)</label>
          <div style={{ position: 'relative' }}>
      ...
      <div style={{ borderTop: '1px solid var(--border-color)', marginTop: '2rem', paddingTop: '1.2rem', textAlign: 'center', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
        ...
        <span onClick={onSwitchToLogin} style={{ color: 'var(--primary)', fontWeight: 'bold', cursor: 'pointer' }}>
          Log In
        </span>
      </div>
    </div>
