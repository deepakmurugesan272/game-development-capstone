...
import { UserPlus, ShieldAlert, Mail, User, Key, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';

export const Register: React.FC<{ onSwitchToLogin: () => void; onSuccess: () => void }> = ({ onSwitchToLogin, onSuccess }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '80vh', justifyContent: 'center', alignItems: 'center', padding: '1rem' }}>
      ...
      <div style={{ 
        ...
        {/* Right Panel: Ancient Scroll Register Form */}
        <div 
        ...
        >
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem', position: 'relative' }}>
            ...
            <p style={{ color: '#5c4033', fontSize: '0.88rem', fontWeight: 700, marginTop: '0.3rem' }}>
              {language === 'en' ? 'Create a profile to log scores' : 'புதிய கணக்கைத் தொடங்கவும்'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                <User size={16} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#8b5a2b' }} />
                <input
                  type="text"
                  placeholder={language === 'en' ? 'e.g. kboom' : 'பயனர் பெயர்'}
                  value={username}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  type="email"
                  placeholder={language === 'en' ? 'you@example.com' : 'மின்னஞ்சல்'}
                  value={email}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#5c4033' }}>
                {language === 'en' ? 'Password (Min 6 chars)' : 'கடவுச்சொல் (குறைந்தது 6 எழுத்துக்கள்)'}
              </label>
              <div style={{ position: 'relative' }}>
            ...
            {/* Unlocking animation when loading */}
            <button
              type="submit"
              disabled={loading}
            ...
            >
              {loading ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ color: '#3b2005' }}>{language === 'en' ? 'Creating Scroll...' : 'கணக்கு உருவாகிறது...'}</span>
                </div>
              ) : (
                <span style={{ color: '#3b2005' }}>{language === 'en' ? 'Create Account' : 'கணக்கை உருவாக்கு'}</span>
              )}
            </button>
          </form>
          ...
          <div style={{ borderTop: '1px solid rgba(139,74,29,0.2)', marginTop: '2rem', paddingTop: '1.2rem', textAlign: 'center', fontSize: '0.88rem', color: '#5c4033', fontWeight: 700 }}>
            ...
            <span onClick={onSwitchToLogin} style={{ color: 'var(--primary)', fontWeight: 800, cursor: 'pointer', textDecoration: 'underline' }}>
              {language === 'en' ? 'Log In' : 'உள்நுழையவும்'}
            </span>
          </div>
        </div>
